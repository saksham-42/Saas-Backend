...
def test_admin_can_delete_user(client, admin_token):
    ...
    users = client.get("/api/v1/users/", headers=auth_headers(admin_token))
    user_id = next(u["id"] for u in users.json() if u["email"] == "todelete@example.com")
    response = client.delete(f"/api/v1/users/{user_id}", headers=auth_headers(admin_token))
    assert response.status_code == 200


def test_non_admin_cannot_delete_user(client, member_token):
    response = client.delete("/api/v1/users/1", headers=auth_headers(member_token))
    assert response.status_code == 403


def test_non_member_cannot_access_org(client, other_user_token, org):
    response = client.get(f"/api/v1/organization/{org['id']}", headers=auth_headers(other_user_token))
    assert response.status_code == 403


def test_non_member_cannot_access_org_tasks(client, other_user_token, org):
    response = client.get(f"/api/v1/organization/{org['id']}/tasks", headers=auth_headers(other_user_token))
    assert response.status_code == 403
...
def test_only_admin_can_add_members(client, admin_token, member_token, org):
    ...
    response = client.post(f"/api/v1/organization/{org['id']}/members?user_id={new_user_id}", headers=auth_headers(member_token))
    assert response.status_code == 403
...
def test_only_admin_can_remove_members(client, admin_token, member_token, org):
    ...
    member_id = next(u["id"] for u in users.json() if u["email"] == "member@example.com")
    client.post(f"/api/v1/organization/{org['id']}/members?user_id={member_id}", headers=auth_headers(admin_token))
    response = client.delete(f"/api/v1/organization/{org['id']}/members/{member_id}", headers=auth_headers(member_token))
    assert response.status_code == 403