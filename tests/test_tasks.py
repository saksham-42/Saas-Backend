...
def test_soft_delete_task_not_in_list(client, admin_token, org, task):
    client.delete(f"/api/v1/organization/{org['id']}/tasks/{task['id']}", headers=auth_headers(admin_token))
    response = client.get(f"/api/v1/organization/{org['id']}/tasks", headers=auth_headers(admin_token))
...
def test_pagination_skip(client, admin_token, org):
    for i in range(3):
        client.post(f"/api/v1/organization/{org['id']}/tasks", json={
            "title": f"Task {i}", "description": None,
            "status": "pending", "priority": "medium",
            "assigned_to": None, "due_date": None
        }, headers=auth_headers(admin_token))
    response = client.get(f"/api/v1/organization/{org['id']}/tasks?skip=2&limit=10", headers=auth_headers(admin_token))
    assert response.status_code == 200
...
def test_pagination_limit(client, admin_token, org):
    ...
    response = client.get(f"/api/v1/organization/{org['id']}/tasks?skip=0&limit=3", headers=auth_headers(admin_token))
    assert response.status_code == 200
...
def test_update_task_status(client, admin_token, org, task):
    response = client.put(f"/api/v1/organization/{org['id']}/tasks/{task['id']}", json={
        "status": "completed"
    }, headers=auth_headers(admin_token))
    assert response.status_code == 200
...
def test_get_tasks_filter_by_status(client, admin_token, org):
    ...
    response = client.get(f"/api/v1/organization/{org['id']}/tasks?status=pending", headers=auth_headers(admin_token))
    assert response.status_code == 200
...
def test_assign_task(client, admin_token, member_token, org, task):
    ...
    client.post(f"/api/v1/organization/{org['id']}/members?user_id={member_id}", headers=auth_headers(admin_token))
    response = client.patch(f"/api/v1/organization/{org['id']}/tasks/{task['id']}/assign", json={
...
def test_delete_already_deleted_task(client, admin_token, org, task):
    client.delete(f"/api/v1/organization/{org['id']}/tasks/{task['id']}", headers=auth_headers(admin_token))
    response = client.delete(f"/api/v1/organization/{org['id']}/tasks/{task['id']}", headers=auth_headers(admin_token))
    assert response.status_code == 404
...
def test_get_tasks_filter_in_progress(client, admin_token, org):
    ...
    response = client.get(f"/api/v1/organization/{org['id']}/tasks?status=in_progress", headers=auth_headers(admin_token))
    assert response.status_code == 200
