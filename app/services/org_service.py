...
def create_organization(org: Org_create, db: Session, current_user):
    "Create a new organization and add the creator as an admin member. Raises 400 if slug is taken."
...
def add_members(org_id: int, db: Session, user_id: int, member):
    "Add a user to an organization. Requires admin role. Raises 400 if already a member."
...
def get_members(db: Session, org_id: int, limit: int, skip: int):
    "Fetch an organization by ID. Raises 404 if not found."
...
def update_member_info(org_id: int, user_id: int, role: Memb_role_update, db: Session, member):
    "Update a member's role. Requires admin. Raises 400 if target is the org owner."
    if member.role != "admin":
...
def remove_member(db: Session, org_id: int, user_id: int, member):
    "Remove a member from an organization. Requires admin. Raises 400 if target is the org owner."
    if member.role != "admin":
