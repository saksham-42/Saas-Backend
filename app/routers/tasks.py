...
def create_tasks(org_id: int, task: Task_create, member: OrganizationMember = Depends(
...
def update_task(org_id: int, task_id: int, task_update: TaskUpdate,
                member: OrganizationMember = Depends(get_org_member), db: Session = Depends(get_database)):
...
def assign_task(org_id: int, task_id: int, task_assign: TaskAssign,
                member: OrganizationMember = Depends(get_org_member), db: Session = Depends(get_database)):
    return task_service.assign_task(org_id, task_id, task_assign, db)


@router.delete("/{org_id}/tasks/{task_id}", status_code=200)
def delete_task(org_id: int, task_id: int, member: OrganizationMember = Depends(
        get_org_member), db: Session = Depends(get_database)):
